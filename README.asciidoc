...
'git-remote-hg' consists of a single Python script. It couldn't get simpler.

=== Gentoo ===

Under Gentoo Linux, add the third-party
https://github.com/leycec/raiagent[raiagent] overlay and emerge 'git-remote-hg':

--------------------------------------
layman -a raiagent
emerge --autounmask-write git-remote-hg
--------------------------------------

=== Manual ===

On other platforms, manually install both Python 2 and Mercurial and download
the latest 'git-remote-hg' script to any directory in your `$PATH`: e.g.,
...
request](https://github.com/fingolfin/git-remote-hg/pulls). And thanks!
